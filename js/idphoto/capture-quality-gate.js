(function(g){'use strict';
  ...
  function grayStats(ctx,x,y,w,h){
    const d=ctx.getImageData(x,y,w,h).data; let n=0,sum=0,sum2=0,edge=0,prev=null;
    for(let i=0;i<d.length;i+=16){const lum=.2126*d[i]+.7152*d[i+1]+.0722*d[i+2];sum+=lum;sum2+=lum*lum;n++;if(prev!=null)edge+=Math.abs(lum-prev);prev=lum;}
    const mean=n?sum/n:0, variance=n?sum2/n-mean*mean:0; return {mean,variance,edge:n?edge/n:0};
  }
  function assess({imageWidth,imageHeight,face,faceCount=1,stats,target={}}){
    ...
    return {ok:!issues.some(i=>i.severity==='block'),score,issues,face:{x:face.x,y:face.y,width:face.width,height:face.height},metrics:{faceHeightRatio:fh,faceWidthRatio:fw,effectiveFacePx,topMarginRatio:top,leftMarginRatio:left,rightMarginRatio:right}};
  }
  async function analyzeImage(img,target={}){
    ...
    return assess({imageWidth:w,imageHeight:h,face:{x,y,width:fw,height:fh},faceCount:faces.length,stats:{...all,shadowDelta:Math.abs(l.mean-r.mean)},target});
  }
