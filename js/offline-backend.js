  ...
  async function handleFetch(url, opts) {
    ...
    if (path === '/dup-check' && method === 'POST') {
      ...
      for (const r of rows) {
        ...
        const rCurrency = String(r.cur || getBaseCurrency()).toUpperCase();
        // 持久化账务金额在写入时已经统一换算为基准币种。
        // 这里严禁根据记录保留的原始 currency 再次换算，否则外币记录会被二次折算。
    ...
    if (path === '/insights/today' && method === 'GET') {
      ...
      for(const p of old){ const age=Math.floor((new Date(today)-new Date(String(p.doc_date).slice(0,10)))/86400000); if(age>=30) out.push({kind:'unpaid',severity:age>=60?'high':'medium',title:`${p.supplier||'供应商'} 未付款`,detail:`剩余 ${getBaseCurrency()} ${money(num(p.total_amount)-num(p.paid_amount)).toFixed(2)} · ${age} 天`,record_id:p.id}); }
    ...
    if (path === '/insights/anomalies' && method === 'GET') {
      ...
      dups.forEach(x=>out.push({kind:'duplicate',severity:'medium',title:'疑似重复支出',detail:`${x.date} · ${getBaseCurrency()} ${money(x.amount).toFixed(2)} · ${x.c} 笔`,date:x.date,amount:money(x.amount),record_ids:String(x.ids||'').split(',').map(Number).filter(Boolean)}));
      ...
      const today=new Date(); old.forEach(p=>{const age=Math.floor((today-new Date(String(p.doc_date).slice(0,10)))/86400000); if(age>=30)out.push({kind:'aged_unpaid',severity:age>=60?'high':'medium',title:'长期未付款',detail:`${p.supplier||'供应商'} · ${age} 天 · ${getBaseCurrency()} ${money(num(p.total_amount)-num(p.paid_amount)).toFixed(2)}`,record_id:p.id});});
      const rows=DB.prepare("SELECT substr(date,1,7) m,category,SUM(amount) s FROM expense WHERE mode=? AND date>=date('now','-120 day') GROUP BY m,category").all(mode); const by={}; rows.forEach(r=>{(by[r.category]||(by[r.category]=[])).push(r)}); const _n=new Date(); const cm=`${_n.getFullYear()}-${String(_n.getMonth()+1).padStart(2,'0')}`; Object.entries(by).forEach(([cat,a])=>{const cur=a.find(x=>x.m===cm); if(!cur)return; const prev=a.filter(x=>x.m!==cm).map(x=>num(x.s)); if(prev.length>=2){const avg=prev.reduce((x,y)=>x+y,0)/prev.length;if(avg>0&&num(cur.s)>avg*1.5&&num(cur.s)-avg>100)out.push({kind:'spike',severity:'medium',title:'分类支出明显增加',detail:`${cat} 本月 ${getBaseCurrency()} ${money(cur.s).toFixed(2)}，高于近月均值 ${Math.round((num(cur.s)/avg-1)*100)}%`,category:cat});}});
