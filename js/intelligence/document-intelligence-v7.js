...
(function (global) {
  ...
  function resolveAmount(result, baseFields, opts) {
    ...
    // V7.1：有些 OCR 的 lines 分段会把 TOTAL 与金额拆开，但 fullText 仍保留相邻内容。
    // 对全文再做一次“强业务标签”扫描，只补候选，不凭空创造金额。
    const full = norm(result && (result.fullText || result.text));
    const strongFullPatterns = [
      { re:/\b(?:TOTAL\s+A\s+PAGAR|TOTAL\s+A\s+COBRAR|TOTAL\s+A\s+COBRAR|IMPORTE\s+COBRADO|VALOR\s+(?:DE\s+)?PAGO)\b[^0-9]{0,16}(?:[$¥€£￥₩]\s*)?(-?\d{1,3}(?:[ ,]\d{3})*(?:[.,]\d{2})|-?\d+[.,]\d{2}|-?\d+)/ig, role:'total_due', score:1.70 },
      { re:/(?:^|\s)TOTAL\s*[:=.-]?\s*(?:[$¥€£￥₩]\s*)?(-?\d{1,3}(?:[ ,]\d{3})*(?:[.,]\d{2})|-?\d+[.,]\d{2})/ig, role:'total', score:1.30 },
    ];
    for (const pat of strongFullPatterns) {
      let fm; while ((fm = pat.re.exec(full))) add(fm[1], { score:pat.score, reasons:['fulltext-strong-label'], roles:new Set([pat.role]), lineText:fm[0] });
    }
  ...
  function resolveMerchant(result, baseFields, opts) {
    ...
    // V7.1：票头品牌/机构名优先。避免把底部客服句、地址、付款状态整句当商户。
    const fullText = norm(result && (result.fullText || result.text));
    const known = fullText.match(/\b(OXXO|PETROMAX|CFE|CESPM|EL\s+FLORIDO|CAMPUS\s+[A-Z0-9]{1,8})\b/i);
    if (known) add(known[1].toUpperCase(), 1.52, ['explicit-brand'], null);
    ...
    lines.forEach((line,i)=>{
      ...
      if(t.length>42) score-=0.55;
      if(/\b(?:telefono|tel[eé]fono|contacte|atenci[oó]n|gracias|calle|colonia|av\.?|blvd\.?|cp\.?|c[oó]digo\s+postal)\b/i.test(t)) score-=0.48;
      if(/\b(?:escucha|transacci[oó]n|exitosa|pagado|saldo|cliente|cajero)\b/i.test(t)) score-=0.45;
