...
(function (global) {
  ...
  const REQUIRED_TABLES = [
    ...
    'document_templates', 'merchant_category_rules', 'accounting_audit_log', 'field_resolution_rules', 'ledger_trash', 'internal_transfers', 'purchase_payments', 'recurring_runs',
  ];
  // 必要字段（关键列存在性）
  const REQUIRED_COLUMNS = {
    ...
    internal_transfers: ['date','from_account','to_account','amount','mode'],
    purchase_payments: ['purchase_id','pay_date','amount','mode'],
    recurring_runs: ['rule_key','due_date','record_type','mode'],
  };
  ...
  function ledgerDiagnostics(db) {
    ...
    try {
      const r = db.exec("SELECT COUNT(*) FROM ledger_trash WHERE original_table NOT IN ('income','expense','purchase','internal_transfers') OR record_json='' OR record_json IS NULL");
