...
function render(){
 let p=pet();
 if(!p){p=document.createElement('button');p.id='smDesktopPet';p.className='sm-pet sm-pet-v2';p.type='button';p.innerHTML='<span class="sm-pet-life-badge"></span><span class="sm-pet-action-stage" aria-hidden="true"></span><img class="sm-pet-img" alt=""><span class="sm-pet-bubble"></span>';document.body.appendChild(p);bindPointer(p);p.addEventListener('click',()=>{if(drag&&drag.moved)return;react('happy','我在这里呀！')})}
