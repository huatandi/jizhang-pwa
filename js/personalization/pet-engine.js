...
function render(){
 let p=pet();
 if(!p){p=document.createElement('button');p.id='smDesktopPet';p.className='sm-pet sm-pet-v2';p.type='button';p.innerHTML='<img class="sm-pet-img" alt="">';document.body.appendChild(p);bindPointer(p);p.addEventListener('click',()=>{if(drag&&drag.moved)return})}
...
function react(){/* V222 quiet companion: no speech bubble or business prompt */}
function setLifeScene(){/* V222: life-action presentation intentionally retired */}
...
function scheduleWander(){clearTimeout(walkTimer);/* V222: stay put by default */}
function setPet(v){if(animals.some(a=>a[0]===v)){state.pet=v;save();render()}}
...
function bindPointer(p){let hold=0;p.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'&&e.button!==0)return;drag={id:e.pointerId,sx:e.clientX,sy:e.clientY,px:p.getBoundingClientRect().left,py:p.getBoundingClientRect().top,moved:false};p.setPointerCapture&&p.setPointerCapture(e.pointerId);clearTimeout(hold)});p.addEventListener('pointermove',e=>{if(!drag||drag.id!==e.pointerId)return;const dx=e.clientX-drag.sx,dy=e.clientY-drag.sy;if(Math.hypot(dx,dy)>7)drag.moved=true;if(drag.moved)moveTo(drag.px+dx,drag.py+dy,{transient:true})});const up=e=>{clearTimeout(hold);if(!drag||drag.id!==e.pointerId)return;if(drag.moved)save();setTimeout(()=>drag=null,0)};p.addEventListener('pointerup',up);p.addEventListener('pointercancel',up)}
