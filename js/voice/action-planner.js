...
(function(global){
  ...
  function planOne(raw,ctx){
    ...
    for(const k of ['income','expense']) for(const a of A[k]){
      const z=compact(a); if(x===z||['切换到','切换','进入','打开','选择','选','转到'].some(v=>x===compact(v)+z)){
        actions.push({type:'NAVIGATE',value:k}); return {actions,commandLike:true,confidence:.99};
      }
    }
    ...
    if(A.accountCue.concat(A.amountCue,A.cash,A.other,A.income,A.expense).some(a=>x.includes(compact(a))) || /^(切换|进入|打开|选择|选|改|设置|设为|用|使用)/.test(x)) commandLike=true;
  ...
  global.VoiceActionPlanner={VERSION:1,ALIASES:A,plan,compact};
